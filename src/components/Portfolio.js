 ...
 * Single-file React component. No external dependencies beyond React itself.
 *
 * Usage:
 *   1. npx create-react-app my-portfolio
 *   2. Replace src/App.js content with this file, OR rename to Portfolio.jsx
 *      and import it in App.js: import Portfolio from './Portfolio'; export default Portfolio;
 *   3. npm start
 *
 * Fonts are loaded via Google Fonts (requires internet connection).
 */
...
/* grid overlay */
.port::after{content:'';position:fixed;inset:0;background-image:linear-gradient(rgba(124,111,255,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(124,111,255,.04) 1px,transparent 1px);background-size:80px 80px;pointer-events:none;z-index:0;}
...
.cursor-ring{display:none;position:fixed;width:36px;height:36px;border:1px solid rgba(124,111,255,.5);border-radius:50%;pointer-events:none;z-index:9998;transform:translate(-50%,-50%);transition:width .25s,height .25s,border-color .2s;}
...
/* nav */
.topnav{position:fixed;top:0;left:0;right:0;z-index:900;border-bottom:1px solid var(--border);backdrop-filter:blur(24px);background:rgba(6,6,8,.85);}
...
.project-card{background:var(--bg);padding:36px;position:relative;overflow:hidden;transition:background .3s;height:100%;}
...
.tag{font-family:var(--font-mono);font-size:9px;letter-spacing:.08em;text-transform:uppercase;color:var(--muted);border:1px solid var(--border2);padding:4px 10px;border-radius:2px;}
...
const SECTIONS = ['hero', 'projects', 'stack', 'experience', 'about', 'contact'];

const TICKER_ITEMS = [
  'React.js','Next.js','React Native','Expo','Node.js','Firebase','TypeScript',
  'Tailwind CSS','Framer Motion','Vercel','GraphQL','Cloudinary','EAS Build','Figma → Code',
];

const FEATURED_PROJECT = {
  index: '[RE_01] — FLAGSHIP · MOBILE',
  name: 'OMI Health',
  ...
  tags: ['React Native', 'Expo', 'Firebase', 'Firestore', 'Node.js', 'EAS Build'],
...
const PROJECTS = [
  {
    id: '[RE_02] · FINTECH · MOBILE', name: 'VODIUM',
    ...
    link: '#', linkLabel: 'Case study coming →',
  },
  {
    id: '[RE_03] · WEB · COMMUNITY', name: 'DU Alumni',
    desc: 'Alumni engagement platform with Cloudinary media, event listings, member directories, and editorial CMS integration.',
    tags: ['React', 'Next.js', 'Cloudinary', 'CMS'],
    link: 'https://du-alumni-steel.vercel.app/', linkLabel: 'View live →',
  },
  {
    id: '[RE_04] · WEB · REAL ESTATE', name: 'Hillstar',
    desc: 'Premium property listing platform — headless CMS, virtual tour embeds, lead capture, and SSG on Vercel Edge for sub-2s loads.',
    tags: ['React', 'Headless CMS', 'SSG', 'Vercel Edge'],
    link: 'https://hillstar-realestate.vercel.app/', linkLabel: 'View live →',
  },
];

const STACK = [
  { layer: 'Layer 01 / Frontend', title: 'Web UI',        items: ['React.js & Next.js 14', 'TypeScript', 'Tailwind CSS', 'Framer Motion', 'Chakra UI'] },
  { layer: 'Layer 02 / Mobile',   title: 'Native Apps',   items: ['React Native (Expo)', 'EAS Build & Submit', 'Reanimated 3', 'Expo Router', 'Native Modules'] },
  { layer: 'Layer 03 / Backend',  title: 'Server & Data', items: ['Node.js + Express', 'Firebase / Firestore', 'Serverless Functions', 'REST & GraphQL', 'WebSockets'] },
  { layer: 'Layer 04 / Infra',    title: 'Deploy & Scale',items: ['Vercel (Edge / ISR)', 'Cloudinary CDN', 'GitHub Actions CI', 'App & Play Store', 'Lighthouse 95+ perf'] },
];

const SKILLS = [
  'React Query','Zustand','Redux Toolkit','Zod','React Hook Form','Storybook',
  'Vitest','Playwright','Prisma','Supabase','Push Notifications','Deep Linking',
  'Paystack','Flutterwave','Stripe','Mapbox','ML Kit','OpenAI API','Figma → Code','WCAG A11y',
];

const EXPERIENCE = [
  { date: '2023 — Present', role: 'Senior Full Stack Engineer', company: '// FREELANCE & CONTRACT', body: 'Delivering 10+ web and mobile solutions for startups and SMEs across health, fintech, agritech, and real estate. Owning architecture decisions, CI/CD pipelines, and App Store releases end-to-end.', current: true },
  { date: '2022 — 2023', role: 'React Native Developer', company: '// VODIUM', body: 'Built and maintained a cross-platform fintech application. Led mobile architecture, biometric auth, encrypted storage, and Play/App Store release pipelines.' },
  { date: '2021 — 2022', role: 'Frontend Developer', company: '// HILLSTAR GROUP', body: 'Developed a high-performance real estate platform from scratch. Sub-2s page loads using React + headless CMS + SSG on Vercel Edge.' },
];

const LEVELING_UP = ['System Design', 'Rust', 'AWS Fundamentals', 'tRPC', 'Bun.js', 'LLM integrations', 'Docker', 'Kubernetes basics'];

const PHILOSOPHY = [
  { label: 'Performance First', text: 'Every millisecond counts. I write code with Lighthouse scores, bundle sizes, and real-world network conditions in mind from the first commit.' },
  { label: 'Architecture Over Hacks', text: 'I resist the quick fix. Scalable, readable, maintainable architecture is the product — not just the code that ships it.' },
  { label: 'Design Literacy', text: 'I read Figma files fluently and bridge the design-engineering gap with precision. No "close enough" compromises.' },
  { label: 'Ownership Mentality', text: 'I treat every project like my name is on it. Because it is.' },
];
...
function FadeUp({ children, delay = 0, className = '' }) {
  ...
  return (
    <div ref={ref} className={`fade-up${vis ? ' visible' : ''} ${className}`} style={{ transitionDelay: `${delay}s` }}>
      {children}
    </div>
  );
}
  ...
  return (
    <>
      <nav className="topnav" aria-label="Main navigation">
        <div className="nav-inner">
          <a href="#hero" className="nav-logo">
            <span className="acc">❯</span> ade_engine<span className="acc">.sh</span>
          </a>
      ...
      <div className={`mobile-menu${open ? ' open' : ''}`} aria-hidden={!open}>
        {NAV_LINKS.map(l => <a key={l.label} href={l.href} onClick={close}>{l.label}</a>)}
      </div>
    </>
  );
}
  ...
  return (
    <div className="terminal-card">
      ...
      <div className="terminal-body">
        <div><span className="t-prompt">ade@engine</span> ~ $ whoami</div>
        <div className="t-out"><span className="t-acc2">→</span> Senior Software Engineer</div>
        <br />
        <div><span className="t-prompt">ade@engine</span> ~ $ cat stats.json</div>
        <div className="t-out">{'{'}</div>
        <div className="t-out">&nbsp;&nbsp;<span className="t-acc">"shipped"</span>: 20,</div>
        <div className="t-out">&nbsp;&nbsp;<span className="t-acc">"stack"</span>: ["React","RN","Node","Firebase"],</div>
        <div className="t-out">&nbsp;&nbsp;<span className="t-acc">"platforms"</span>: ["Web","iOS","Android"],</div>
        <div className="t-out">&nbsp;&nbsp;<span className="t-acc2">"status"</span>: <span className="t-warn">"AVAILABLE"</span></div>
        <div className="t-out">{'}'}</div>
        <br />
        <div><span className="t-prompt">ade@engine</span> ~ $ <span className="t-cursor" /></div>
      </div>
    </div>
  );
}
        ...
        <div className="hero-grid">
          <div>
            ...
            <p className="hero-desc">
              I build performant, production-grade web and mobile systems — from zero to scale.
              20+ shipped products, clean architecture, relentless attention to craft.
            </p>
            ...
            <div className="hero-ctas">
              ...
              <a href="mailto:adeyanjuolamilekan080@gmail.com" className="btn-ghost">Get in touch →</a>
            </div>

            <div className="hero-stats">
              {[['20+', 'Shipped'], ['5yr', 'Experience'], ['99%', 'Uptime']].map(([n, l]) => (
  ...
  return (
    <section className="section" id="projects">
      <div className="container">
        ...
        <FadeUp delay={0.1} className="metric-row">
          {[['20+','Production apps'],['100K+','Lines prod code'],['4★','Avg store rating'],['5ms','Avg API latency']].map(([v, k]) => (
        ...
        <FadeUp delay={0.2}>
          <div className="project-featured">
            <div className="project-featured-content">
              <div className="project-index">{FEATURED_PROJECT.index}</div>
              <h3 className="project-name">{FEATURED_PROJECT.name}</h3>
              <p className="project-desc">{FEATURED_PROJECT.desc}</p>
              <div className="project-tags">
                {FEATURED_PROJECT.tags.map(t => <span key={t} className="tag">{t}</span>)}
              </div>
              <a href={FEATURED_PROJECT.link} className="project-link" target="_blank" rel="noopener noreferrer">
                {FEATURED_PROJECT.linkLabel}
              </a>
            </div>
            <div className="project-featured-visual">
              <div className="live-badge">{FEATURED_PROJECT.live}</div>
              <div className="arch-box">
                <div style={{ color: 'var(--accent2)', marginBottom: 10, fontSize: 10 }}>[ APP_ARCHITECTURE ]</div>
    ...
    <section className="section bg2 bordered" id="stack">
      <div className="container">
        <FadeUp className="section-header">
          ...
          <h2 className="section-title">The <em>arsenal</em></h2>
        </FadeUp>
          ...
          {STACK.map(s => (
            <div key={s.layer} className="stack-cell">
              ...
              <div className="stack-items">
                {s.items.map(item => <div key={item} className="stack-item-row">{item}</div>)}
              </div>
            </div>
          ))}
        </FadeUp>
        ...
        <div className="exp-grid">
          <FadeUp delay={0.1}>
            <div className="timeline">
              {EXPERIENCE.map(e => (
                <div key={e.role} className={`timeline-item${e.current ? ' current' : ''}`}>
          ...
          <FadeUp delay={0.2}>
            <div style={{ marginBottom: 48 }}>
              <div className="section-index" style={{ marginBottom: 24 }}>Education</div>
              <div className="timeline-role" style={{ marginBottom: 6 }}>B.Sc. Computer Science</div>
              <div className="timeline-company" style={{ marginBottom: 12 }}>// DOMINION UNIVERSITY</div>
              <p className="timeline-body">
                Algorithms, systems design, and software engineering. Led build sessions, code reviews,
                and open-source initiatives as an active member of the university tech community.
              </p>
            </div>

            <div>
              <div className="section-index" style={{ marginBottom: 24 }}>Currently leveling up</div>
              <div className="skills-flow" style={{ marginTop: 0 }}>
                {LEVELING_UP.map(s => <span key={s} className="skill-pill">{s}</span>)}
              </div>
            </div>
          </FadeUp>
        </div>
        ...
        <div className="philosophy-grid">
          <FadeUp delay={0.1}>
            <p className="philosophy-text">
              I believe great software is <em>invisible</em>. The best interface is the one users never
              have to think about. I obsess over the gap between "it works" and "it <em>feels</em> right"
              — across 20+ shipped products.
            </p>
          </FadeUp>
...
function Contact() {
  ...
  const submit = e => {
    e.preventDefault();
    // TODO: wire to Formspree → https://formspree.io
    alert('Message received! Wire this to Formspree or your preferred backend.');
  };
    ...
    <section className="section" id="contact">
      <div className="container">
        <FadeUp className="section-header">
          ...
          <h2 className="section-title">Build something <em>great</em></h2>
        </FadeUp>

        <div className="contact-grid">
          <FadeUp delay={0.1}>
            <a href="mailto:adeyanjuolamilekan080@gmail.com" className="contact-email">
              adeyanjuolamilekan080@gmail.com
            </a>
            <div className="contact-links">
              <a href="https://github.com/Ade-yanju" target="_blank" rel="noopener noreferrer" className="social-link">
                <IconGithub /> GitHub
              </a>
          ...
          <FadeUp delay={0.2}>
            <form className="contact-right" onSubmit={submit}>
              <div className="form-group">
                ...
                <input className="form-input" name="name" type="text" placeholder="Your name" value={form.name} onChange={change} />
              </div>
              <div className="form-group">
                <label className="form-label">Email</label>
                <input className="form-input" name="email" type="email" placeholder="your@email.com" value={form.email} onChange={change} />
              </div>
              <div className="form-group">
                <label className="form-label">Message</label>
                <textarea className="form-textarea" name="message" rows={4} placeholder="Tell me about your project..." value={form.message} onChange={change} />
              </div>
  ...
  return (
    <footer>
      <div className="footer-inner">
        <div className="footer-copy">© 2026 Olamilekan Ogunyade. Built from scratch.</div>
        <div className="footer-right">
          <span>{time}</span>
          <a href="https://github.com/Ade-yanju" target="_blank" rel="noopener noreferrer">/github</a>
